...
function verifyHome(string $tipoUsuario)
{
    ...
    if (!isset($_SESSION[Consts::SESSION_KEY_USER])) {
        ...
        echo '<script type="text/javascript">
                alertErrorRedirect(CONST_MSG_ALERT.USER_NOT_SESSION.TITLE, CONST_MSG_ALERT.USER_NOT_SESSION.TEXT, WEB_URL.VIEW_LOGIN);
            </script>';
        die();
    }
    ...
    if ($_SESSION[Consts::SESSION_KEY_USER][Consts::SESSION_USER_KEY_TIPO] != $tipoUsuario) {
        require_once(dirname(__FILE__) . '/scripts.php');
        echo '<script type="text/javascript">
                alertErrorRedirect(CONST_MSG_ALERT.PERMISSIONS_DENIED.TITLE, CONST_MSG_ALERT.PERMISSIONS_DENIED.TEXT, WEB_URL.VIEW_LOGIN);
            </script>';
        die();
    }
}
